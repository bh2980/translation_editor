...
export function StatusManager({
  ...
  const [local, setLocal] = useState(
    initialStatuses.length > 0 ? initialStatuses : defaultStatuses
  );

  function update(idx: number, patch: Partial<any>) {
  ...
  return (
    <div className="space-y-3">
      {local.map((s, idx) => (
        ...
        >
          <div
            className={`h-4 w-4 rounded-full ${colorToClass(s.color)}`}
            aria-hidden
          />
          <Input
